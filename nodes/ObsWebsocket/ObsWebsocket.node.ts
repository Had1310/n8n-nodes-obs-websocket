...
export class ObsWebsocket implements INodeType {
  description: INodeTypeDescription = {
    ...
    properties: [
        ...
        options: [
          {
            name: 'Send Message',
            value: 'sendMessage',
            description: 'Send message to Advanced Scene Switcher',
            action: 'Send vendor message',
          },
          {
            name: 'Run Macro',
            value: 'runMacro',
            description: 'Run a SceneSwitcher macro directly',
            action: 'Run SceneSwitcher macro',
          },
        ],
        default: 'sendMessage',
      },
      {
        ...
        default: 'AdvancedSceneSwitcher',
        required: true,
        description: 'The vendor name (e.g., "AdvancedSceneSwitcher")',
      },
      {
        ...
        default: 'AdvancedSceneSwitcherMessage',
        required: true,
      ...
      },
      {
        ...
        default: '',
        required: true,
        description: 'Message to send (e.g., "claveaqui")',
      },
      {
        ...
        default: '',
        required: true,
        description: 'Name of the SceneSwitcher macro to run',
      },
    ],
  };
  ...
  async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
    ...
    let cleanHost = host.replace(/^(wss?|https?):\/\//, '').replace(/\/$/, '');
        ...
        try {
          ...
          // ==================== VIRTUAL CAMERA ====================
            ...
            if (operation === 'getStatus') {
              ...
              responseData = { active: status.outputActive };
            } else if (operation === 'start') {
          ...
          // ==================== AUDIO ====================
            ...
            if (operation === 'getMuteStatus') {
              const muteStatus = await obs.call('GetInputMute', { inputName: audioSourceName });
              responseData = {
                sourceName: audioSourceName,
                muted: muteStatus.inputMuted,
              };
            } else if (operation === 'mute') {
              await obs.call('SetInputMute', { inputName: audioSourceName, inputMuted: true });
              responseData = {
                success: true,
              ...
              await obs.call('SetInputMute', { inputName: audioSourceName, inputMuted: false });
              responseData = {
                success: true,
              ...
              await obs.call('ToggleInputMute', { inputName: audioSourceName });
              const newState = await obs.call('GetInputMute', { inputName: audioSourceName });
              responseData = {
                success: true,
              ...
              const volumeInfo = await obs.call('GetInputVolume', { inputName: audioSourceName });
          ...
          // ==================== FILTER ====================
            ...
            if (operation === 'list') {
              const filters = await obs.call('GetSourceFilterList', { sourceName: sourceNameForFilter });
              responseData = {
          ...
          // ==================== TRANSITION ====================
            ...
            if (operation === 'getCurrent') {
              ...
              responseData = { success: true, message: `Transition set to: ${transitionName}` };
            } else if (operation === 'trigger') {
              await obs.call('TriggerStudioModeTransition');
              responseData = { success: true, message: 'Studio mode transition triggered' };
            }
          }
          ...
          // ==================== MEDIA ====================
            ...
            if (operation === 'getStatus') {
              const status = await obs.call('GetMediaInputStatus', { inputName: mediaSourceName });
              responseData = {
              ...
              await obs.call('SetMediaInputCursor', { inputName: mediaSourceName, mediaCursor: 0 });
              await obs.call('TriggerMediaInputAction', {
                inputName: mediaSourceName,
                mediaAction: 'OBS_WEBSOCKET_MEDIA_INPUT_ACTION_PLAY',
              });
              responseData = { success: true, message: `Media "${mediaSourceName}" playing` };
            } else if (operation === 'pause') {
              await obs.call('TriggerMediaInputAction', {
                inputName: mediaSourceName,
                mediaAction: 'OBS_WEBSOCKET_MEDIA_INPUT_ACTION_PAUSE',
              });
              responseData = { success: true, message: `Media "${mediaSourceName}" paused` };
            } else if (operation === 'restart') {
              await obs.call('TriggerMediaInputAction', {
                inputName: mediaSourceName,
                mediaAction: 'OBS_WEBSOCKET_MEDIA_INPUT_ACTION_RESTART',
              });
              responseData = { success: true, message: `Media "${mediaSourceName}" restarted` };
            } else if (operation === 'stop') {
              await obs.call('TriggerMediaInputAction', {
                inputName: mediaSourceName,
                mediaAction: 'OBS_WEBSOCKET_MEDIA_INPUT_ACTION_STOP',
              });
              responseData = { success: true, message: `Media "${mediaSourceName}" stopped` };
            } else if (operation === 'next') {
              await obs.call('TriggerMediaInputAction', {
                inputName: mediaSourceName,
                mediaAction: 'OBS_WEBSOCKET_MEDIA_INPUT_ACTION_NEXT',
              });
              responseData = { success: true, message: `Media "${mediaSourceName}" skipped to next` };
            } else if (operation === 'previous') {
              await obs.call('TriggerMediaInputAction', {
                inputName: mediaSourceName,
                mediaAction: 'OBS_WEBSOCKET_MEDIA_INPUT_ACTION_PREVIOUS',
              });
              responseData = { success: true, message: `Media "${mediaSourceName}" went to previous` };
            }
          }
          ...
          // ==================== TEXT SOURCE ====================
            ...
            if (operation === 'setText') {
              ...
              const currentSettings = await obs.call('GetInputSettings', { inputName: textSourceName });
              ...
              const settings = await obs.call('GetInputSettings', { inputName: textSourceName });
              responseData = {
                sourceName: textSourceName,
          ...
          // ==================== HOTKEY ====================
            ...
            if (operation === 'trigger') {
              ...
              await obs.call('TriggerHotkeyByName', { hotkeyName: hotkeyName });
              responseData = {
                success: true,
              ...
              responseData = { hotkeys: hotkeys.hotkeys };
            }
          }
          ...
          // ==================== PROFILE ====================
            ...
            if (operation === 'getCurrent') {
              ...
              responseData = { currentProfile: profile.parameterValue };
            } else if (operation === 'list') {
              ...
              responseData = { success: true, message: `Switched to profile: ${profileName}` };
            } else if (operation === 'getSceneCollection') {
              const collection = await obs.call('GetSceneCollectionList');
              responseData = { currentSceneCollection: collection.currentSceneCollectionName };
              ...
              responseData = { success: true, message: `Switched to scene collection: ${sceneCollectionName}` };
            }
          }
          ...
          // ==================== VENDOR (NUEVO) ====================
            ...
            if (operation === 'sendMessage') {
              ...
              responseData = {
                success: true,
                message: `Vendor message sent to ${vendorName}`,
                vendorName: vendorName,
                requestType: requestType,
                requestData: { message: message },
              };
            } else if (operation === 'runMacro') {
              const macroName = this.getNodeParameter('macroName', i) as string;
              await obs.call('CallVendorRequest', {
                vendorName: vendorName,
              ...
              responseData = {
                success: true,
                message: `Macro "${macroName}" executed via SceneSwitcher`,
                vendorName: vendorName,
                macroName: macroName,
              };
            }
